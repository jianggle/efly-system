...
class RoleModel extends BaseModel {
  ...
  getRolesByUserId(userId, justValid = false) {
    ...
    return this.query(
      `SELECT a.role_id,b.role_name FROM ${dbTables.SYSTEM_USER_ROLE} AS a,${this.table} AS b WHERE ${condition}`
    )
  }
  ...
  async updateUserRole(userId, ids) {
    ...
    const oldRoles = await this.query(
      `SELECT role_id FROM ${dbTables.SYSTEM_USER_ROLE} WHERE user_id=${userId}`
    )
    ...
    if (addIds.length) {
      ...
      await this.query(
        `INSERT INTO ${dbTables.SYSTEM_USER_ROLE} (user_id, role_id) VALUES ${addValues.join(',')}`
      )
    }
    for (let role_id of delIds) {
      await this.query(
        `DELETE FROM ${dbTables.SYSTEM_USER_ROLE} WHERE user_id=${userId} AND role_id=${role_id}`
      )
    }
  }
  ...
  async getRoleMenu(roleId) {
    const arr = await this.query(
      `SELECT menu_id FROM ${dbTables.SYSTEM_ROLE_MENU} WHERE role_id=${roleId}`
    )
  ...
  async updateRoleMenu(roleId, ids) {
    ...
    if (addIds.length) {
      ...
      await this.query(
        `INSERT INTO ${dbTables.SYSTEM_ROLE_MENU} (role_id, menu_id) VALUES ${addValues.join(',')}`
      )
    }
    for (let menu_id of delIds) {
      await this.query(
        `DELETE FROM ${dbTables.SYSTEM_ROLE_MENU} WHERE role_id=${roleId} AND menu_id=${menu_id}`
      )
    }
  }
  ...
  deleteRoleMenu(menuIds = []) {
    return this.query(
      `DELETE FROM ${dbTables.SYSTEM_ROLE_MENU} WHERE menu_id in (${menuIds.toString()})`
    )
  }
}
