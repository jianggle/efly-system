...
import SysMenuModel from '../model/SysMenuModel.js'
...
const checkSystemUser = async (userId) => {
  const result = await SysUserModel.getOne(userId)
  if (!result) {
...
export const isSuperRole = async (userId) => {
  const userInfo = await SysUserModel.getOne(userId)
  if (!userInfo) throw new ServiceException('用户不存在')
  const userRole = await SysRoleModel.getRolesByUserId(userId, true)
...
const handleEditUser = async (ctx) => {
  ...
  const existItem = await SysUserModel.findOne({ where: { userName, delFlag: 0 } })
  ...
  if (isUpdate) {
    ...
    await SysUserModel.update(params, { userId })
    await SysRoleModel.updateUserRole(userId, role)
  } else {
    if (existItem) {
      throw new ServiceException(repeatMsg)
    }
    ...
    const { insertId } = await SysUserModel.create(params)
    await SysRoleModel.updateUserRole(insertId, role)
  }
...
export const deleteUserAction = async (ctx) => {
  ...
  await checkSystemUser(userId)
  await SysUserModel.update({ delFlag: 1 }, { userId })
  await responseSuccess(ctx)
}
...
export const listUserAction = async (ctx) => {
  ...
  let result = await SysUserModel.getList(status, keyword, offset, limit)
  for (let item of result.rows) {
    item.role = await SysRoleModel.getRolesByUserId(item.userId)
  }
...
export const getUserPermit = async (userId) => {
  ...
  if (checkSuperRole(roleArr)) {
    roleMenus = await SysMenuModel.findAll({ order: SysMenuModel.defaultOrder })
  } else {
    let roleMenuIds = []
    for (let roleId of roleArr) {
      let ids = await SysRoleModel.getRoleMenu(roleId)
      roleMenuIds = [...roleMenuIds, ...ids]
    }
    roleMenuIds = roleMenuIds.filter((item, index, arr) => arr.indexOf(item) === index)
    if (roleMenuIds.length) {
      roleMenus = await SysMenuModel.findAll({
        where: {
          menuId: roleMenuIds,
        },
        order: SysMenuModel.defaultOrder,
      })
    }
  }
...
export const infoUserAction = async (ctx) => {
  const result = await SysUserModel.getOne(ctx.state.user.id)
  if (!result) {
    throw new ServiceException('用户不存在')
  }
  result.role = await SysRoleModel.getRolesByUserId(ctx.state.user.id)
  await responseSuccess(ctx, result)
}
...
export const modifyUserAvatarAction = async (ctx) => {
  ...
  // 更新数据库信息
  await SysUserModel.update({ avatar: result.fileUrl }, { userId: ctx.state.user.id })
  // 删除掉旧的头像
...
export const modifyUserInfoAction = async (ctx) => {
  ...
  await SysUserModel.update({ realName, phone }, { userId: ctx.state.user.id })
  await responseSuccess(ctx)
}
...
export const modifyUserPwdAction = async (ctx) => {
  ...
  const info = await SysUserModel.getOne(userId)
  if (info.password !== encodePassword(oldPwd, info.salt)) {
    throw new ServiceException('旧密码错误')
  }
  await SysUserModel.update({ password: encodePassword(newPwd, info.salt) }, { userId })
  await authLogout(ctx)
  await responseSuccess(ctx)
}
...
export const modifyUserSettingAction = async (ctx) => {
  ...
  await SysUserModel.update({ setting }, { userId: ctx.state.user.id })
  await responseSuccess(ctx)
}
