...
const handleEditMenu = async (ctx) => {
  ...
  const existName = await SysMenuModel.findOne({ where: { menuName } })
  ...
  if (menuType === 'A') {
    ...
    const existPermit = await SysMenuModel.findOne({ where: { permit } })
    ...
    const existPath = await SysMenuModel.findOne({ where: { path } })
  ...
  if (isUpdate) {
    await SysMenuModel.update(params, { menuId })
  } else {
    await SysMenuModel.create(params)
  }
...
export const deleteMenuAction = async (ctx) => {
  ...
  // 执行删除
  await SysMenuModel.destroy({ menuId: ids })
  await SysRoleModel.deleteRoleMenu(ids)
  await responseSuccess(ctx)
}
...
export const modifyMenuOrderAction = async (ctx) => {
  ...
  await SysMenuModel.update({ orderNum }, { menuId })
  await responseSuccess(ctx)
}
