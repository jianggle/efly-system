  ...
  <div class="app-container">
    <el-card>
      <div slot="header">
        ...
        <template v-if="$auth.hasPermit(['blog:link:addCategory'])">
      ...
      <el-table v-loading="isLoading" :data="itemList" border>
        <el-table-column prop="taxis" label="排序" width="80" align="center">
          <template #default="scope">
            <input
              ...
              :disabled="!$auth.hasPermit(['blog:link:orderCategory'])"
        ...
        <el-table-column label="操作" class-name="table-operate-cell" min-width="140">
          <template #default="scope">
            <el-link
              v-if="$auth.hasPermit(['blog:link:modifyCategory'])"
              type="primary"
              icon="el-icon-edit"
              @click="onEdit('modify', scope.row)"
            >修改</el-link>
            <el-popconfirm
              v-if="$auth.hasPermit(['blog:link:deleteCategory'])"
...
import {
  ...
  remove_blog_link_category,
  add_blog_link_category,
  modify_blog_link_category,
  order_blog_link_category,
} from '@/api/blog'
export default {
  name: 'BlogLinkCategory',
  data() {
    return {
      isLoading: false,
      itemList: [],
  ...
  methods: {
    ...
    async onOrderBlur(row, e) {
      ...
      if (!val || !/^\d{1,5}$/.test(val) || val * 1 === this.tempOrderNumber) {
        ...
        await order_blog_link_category({
          catid: row.catid,
          taxis: val * 1
        })
        this.onSuccess()
    ...
    async onRemove({ catid }) {
      try {
        this.isLoading = true
        await remove_blog_link_category({ catid })
        this.onSuccess('删除')
      } catch (error) {
        console.log(error)
      } finally {
    ...
    async onSubmit() {
      try {
        ...
        this.isSubmit = true
        if (this.isAdd) {
          await add_blog_link_category(params)
        } else {
          await modify_blog_link_category(params)
        }
        this.onSuccess()
      } catch (error) {
        console.log(error)
